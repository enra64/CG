...
{
  std::shared_ptr<rt::Camera>   camera    = std::make_shared<rt::PerspectiveCamera>();
  camera->setPosition(rt::Vec3d(8.0,4.0,4.8));
  ...
  if(mesh->loadFromOBJ(gDataPath+fileName))
  {
    ...
    mesh->transform().rotate(150,0,0,1).scale(2,2,2).translate(0,0,2);
...
int main(int argc, char** argv)
{
  ...
  //std::shared_ptr<rt::Scene> scene = makeMeshScene("assets/pwrl.obj",showArrows);
