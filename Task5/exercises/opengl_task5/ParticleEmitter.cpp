...
Particle ParticleEmitter::createParticle() const
{
  ...
  Vec3f sampleDir = Math::samplePhongLobe(mConfig.randomDirectionExponent);
...
void ParticleEmitter::advectParticle(size_t particleIndex, const float dt)
{
  ...
  while (stepCount <mConfig.maxStepIterations && timeLeft > 0.f)
  {
    ...
    bool isect = mCollisionScene->closestIntersection(ray,intersection,maxLambda);
    ...
    { 
      ...
      Vec3f displace = Math::sampleDirectionPhongLobe(p.velocity,mConfig.reflectDeviationExponent);
...
Vec3f ParticleEmitter::sumParticleForces(size_t particleIndex)
{
  Vec3f forces = mConfig.gravity;
